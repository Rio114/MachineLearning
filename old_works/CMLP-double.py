...
def gaborc(H, k):  # H is size of filter, l is direction, k is coeff for sigma
    ...
    for i in range(H):
        for j in range(H):
            buf[i, j] = math.cos((l*(i-H/2)+m*(H/2-j))) * \
                n * math.exp(-((i-H/2)**2+(H/2-j)**2)/(2*H*H))
    return buf
...
def gabors(H, k):  # H is size of filter, l is direction, k is coeff for sigma
    ...
    for i in range(H):
        for j in range(H):
            buf[i, j] = math.sin((l*(i-H/2)+m*(H/2-j))) * \
                n * math.exp(-((i-H/2)**2+(H/2-j)**2)/(2*H*H))
    return buf
...
# differential
    ...
    # 676 dim, d of ReLU, only backprop.
    dXpl2 = np.zeros([bs, nh2, ncv2*mcv2])
    ...
    for e in range(epoch):
        ...
        for b in range(len(pp)):
            ...
            for k in range(nh):
                ...
                Xre[b, k, :], Pos[b, k, :] = relu(
                    Xcv[b, k, :].reshape([1, ncv*mcv]))
                Xpl[b, k, :], WP[b, k, :, :] = pooling(
                    Xre[b, k, :], ncv, mcv, pl, st)
                Xcn[b, k, :] = connorm(Xpl[b, k, :])

            for l in range(nh2):
                for k in range(nh):
                    Xcv2[b, l, :] += WC2[l, :,
                                         :].dot(Xcn[b, k, :].T).T + Bc21[l, :]
                Xre2[b, l, :], Pos2[b, l, :] = relu(
                    Xcv2[b, l, :].reshape([1, ncv2*mcv2]))
                Xpl2[b, l, :], WP2[b, l, :, :] = pooling(
                    Xre2[b, l, :], ncv2, mcv2, pl, st)

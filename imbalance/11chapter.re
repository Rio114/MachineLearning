...
正例・負例、予測の正負の組み合わせの2x2の4マスにデータ数を振り分けることで表を作ることができ、これを混合行列（Confusion Matrix）という。
...
検査の例で考えると、閾値の設定次第で予測の分類が変わってくる。例えば、以下のような架空のパターンが考えられる。
...
//listnum[ex_acc][Accuracyの計算例]{
Accuracy_A = (90 + 9,900) / 10,100 = 0.9891
Accuracy_B = (99 + 9,000) / 10,100 = 0.9009
//}

正の予測が少ないパターンAでは、パターンBよりも負例を言い当てる割合が大きくなるため正解率が上がっている。正例を90当てるか99当てるかよりも、負例を9,900当てるか9,000当てるかの方が正解率に対する影響が大きい。
...
Precisionは正予測の何割が正解であったかという、誤検知の小ささを示す指標である。TP+FPという分母は正の予測の合計であり、分子は正の予測の中の正例の数なので、正予測の言い当て率だといえる。パターンAのように厳選して正の予測を行うなら間違いは少なくなりPrecisionは1に近くなる。逆に多くのものを正だと予測すると、Precisionは元の集団に含まれる正例の割合に近づいていく。

//listnum[ex_pre][Precisionの計算例]{
Precision_A = 90 / (90 + 100) = 0.4738
Precision_B = 99 / (99 + 1,000) = 0.0901
//}

正の予測が少ないパターンAでは言い当てた数（TP）がパターンBよりも少ないが、Precisionで比較するとパターンAの方が大きくなっている。負例を誤って正と予測してしまう、誤検知（FP）が少ないからである。

Precisionでは、正と予測したものだけに着目している。従って、負と予測したものの中に真に正なもの（FN）がたくさんあったとしてもそれはPrecisionからはわからない。
...
Recallは正例の何割を集めることができたという、見逃しの小ささを示す指標である。TPR（True Positive Rate）や再現率とも言う。TP+FNという分母は正例の合計であり、分子は正例の中で正と予測されたものであるため、正例の捕捉率だと言える。見逃しをしたくないがために、何でもかんでも正の予測を行えばRecallは1に近づくが、誤検知（FP）が増えてPrecisionが下がる。一方で、誤検知を減らすために、正の予測が少なくなるようにすると、見逃しが増えるためRecallが下がる。

//listnum[ex_rec][Recallの計算例]{
...
簡単に言うと負例の取りこぼし率。単独ではあまり使うことは無いが、モデルを評価するためROC曲線を描くために算出される。Recallでは正例のうちの正の予測の割合であったが、こちらは負例のうちの正の予測の割合である。見逃しを減らすようにRecallを上げる（＝正例の捕捉率を上げる）と負例を誤って正と予測してしまう割合が増える、つまりFPRが上がる。

==== F1スコア

//listnum[def_f1][f1スコアの計算]{
...
これまで見てきた数値は二値分類の設定だけで機械的に得られる数値に過ぎず、大小に実務的な意味を持たない。
...
合憲損失はパターンAで8,000（利益）、パターンBで-100（損失）となった。当然、利益のほうが目的に適うので今度はパターンAのほうが好ましいという結果になった。この場合、見逃し（FN）には損失が無く、誤検知（FP）にのみ損失が設定されているため、Precisionが高いパターンAが有利になる。
...
クラスごとに計算した評価指標をクラスごとの数で加重平均したもの。

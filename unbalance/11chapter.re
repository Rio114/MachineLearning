...
正例負例、予測の正負の組み合わせの2x2の4マスにデータ数を振り分けることで表を作ることができ、これを混合行列（Confusion Matrix）という。
...
Arrucary（正解率）は一般的に言う精度であり、機械学習の文脈では正解率という。正例は正、負例は負とということをどれだけ正確に言い当てているかという指標である。前述の通り、不均衡なデータに対しては有用な指標とは言えない。
...
Precisionでは、正と予測したものだけに着目している。従って、負と予測したものの中に正例（FN）がたくさんあったとしてもそれはPrecisionからはわからない。

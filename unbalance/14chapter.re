...
@<table>{predicted_ctr}は登録者ごとに補正済みのCTR予測値を広告ごとに算出したものとする。集団alphaやbetaの中での区別をするために、便宜的に１，２，３、、、というように分けている。例えば、alpha@<sub>{1}はalphaの中の>>１さんである。

//table[predicted_ctr][最大予測値による配信]{
集団	CTR@<sub>{A}	CTR@<sub>{B}	CTR@<sub>{C}	CTR@<sub>{D}	平均予測値	最大予測値	配信広告
------------
alpha@<sub>{1}	0.05	0.02	0.02	0.03	0.03	0.05	A
alpha@<sub>{2}	0.03	0.02	0.06	0.01	0.03	0.06	C
alpha@<sub>{3}	0.04	0.05	0.02	0.01	0.03	0.05	B
：	：	：	：	：	：	：	：
Beta@<sub>{1}	0.01	0.01	0.02	0.08	0.03	0.08	B
Beta@<sub>{2}	0.02	0.01	0.02	0.07	0.03	0.07	D
：	：	：	：	：	：	：	：
//}

@<table>{predicted_ctr}ではまだ配信していない広告に対しても予測値を出したことを利用して、登録者ごとに平均予測値や最大予測値を算出している。

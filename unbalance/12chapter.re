...
二値分類モデルは様々な形で構成できる。
例えば、あるデータ点に対して対象となる事象発生の予測確率の大きさを示す値が得られるとき、適当な閾値で集団を分けて閾値より大きな予測値あのものは正、閾値より小さい予測値のものを負と予測することで二値分類を構成できる。
また、ルールベースによりデータの持つ属性がある条件を満たすときに正と予測するとしても二値分類を構成できる。

後者はより一般的な場合であり条件ごとに混合行列を検証しなくてはならないが、
前者は条件が予測値という一次元の数値で表されているので閾値を一次元的に動かすだけで連続的に評価できる。

ここでは前者の評価方法を整理する。
この場合、閾値を決めることで各データ点が混合行列のどこに入るかが決まる。しかし、これではPrecisionやRecallなどの指標は閾値次第で分類の結果が変わってしまうため、複数のモデルを比較できるように閾値に依存しない形でモデルの評価を行いたい。

そこでよく用いられるのがROC曲線やPR曲線、そしてそれらをそれぞれ一つの数字に落とし込んだものがAUCやAPである。
なお、ROC曲線やPR曲線を作成するときには、モデルの予測値は確率のように0から1の範囲である必要はないが、以下では簡単のためモデルの予測値は0から1の範囲の予測確率を示すものとする。
...
@<img>{conf_mat_unbalance}では、少なく正予測を行ったときと多めに正予測をした時の混合行列をイメージしたものである。
マスの大きさが分類されたデータの数を示している。
正例と負例では集団が異なるので同じ閾値を用いても正と予測される割合は異なるが、
二値分類モデルが天邪鬼でない限り、正例の内の正予測の割合が大きくなる。
つまり、FPRよりTPRが大きい。
...
なお、冒頭でも述べたように予測値はは順番に並べることができれば確率値のように0から1の範囲である必要はない。
上記の表の例でいうと、予測値を一律2倍にしてもデータの順番は変わらない。さらに言えば、非線形な変換であっても順序が保たれていれば、ROC曲線は不変である。
逆に言うと、ROC曲線は予測値の順序が正例負例の割合を反映しているかを読み取ることができるものの、絶対値に関する情報を読み取ることができない。

=== AUC
ROCの下側面積Area Under Curveを略してAUCと言い、モデルの良し悪しを一つの数値で表すことができる。
これはTPR、すなわちRecallの平均と見ることができるので、モデルによる正例の捕捉をどれだけ期待できるかを示している。

ポンコツなモデルだとAUCは0.5になる。
つまり、閾値を色々変えて平均してみると半分くらいは正例を捕捉できるということである。
また、一つの閾値で正負を分けることができればAUCは1になる。

AUCが同じなら左のほうの立ち上がりが良いほうが良いモデルである。
なぜならその場合、FPRをあまり上げずにTPRを上げることができているからである。

=== 使用例
正例、負例共に全体の数に対してそれぞれ何割が正と予測されたかを図示したのがROC曲線である。
...
==== PR曲線
PR曲線は@<img>{pr_plot}のようになった。
APはTrainの0.62に対してValidでは0.50である。
また、元の割合で除した平均LiftはTrainの6.18に対してValidでは5.01となった。
 ...
 * 見込みのない人への間違った訴求は10%に抑えたいので、本当にサービス加入してくれるであろう顧客の40%の見逃すのも止む無し、という議論であればROC曲線。
 * サービス加入の可能性が高いと予測した中での無駄打ちを20%程度に抑えたいので、本当にサービス加入してくれるであろう顧客の80%の見逃しには目を瞑るという議論であればPR曲線。

いずれにせよ、誤検知を何に対する割合で見比べるかという点に気を付けたい。

...
一方、一つの閾値で正負を分けることができればAUCは1になる。

AUCが同じなら左のほうの立ち上がりが良いほうが良いモデルである。なぜならその場合、FPRをあまり上げずにTPRを上げることができているからである。

=== 使用例
正例、負例ともに全体の数に対してそれぞれ何割が正と予測されたかを図示したのがROC曲線である。正例を当てることは当然であるが。負例を正しく負と予測したい場合に議論がしやすい。例えば疾患の検査で、罹患していない人に対して誤診断による必要以上の精密検査は損失が大きい、といった状況である。

== Recall-Precision曲線
見逃しをRecallで評価し、正予測に対して誤って正と予測した割合Precisionで誤検知を評価したいときにはRecall-Precision曲線を用いる。

つまり、正例を多く捕捉しつつ、正の予測ではなるべく多くを言い当てたい。ROC曲線とのニュアンスの違いが難しいが、こちらでは負例の数についてはあまり興味がない状況を念頭に置いている。これにより、正例の捕捉割合を上げることで 正の予測の正しさがどれだけ下がるかのトレードオフを視覚化できる。
...
=== 使用例
正例の捕捉率に対して、正の予測の中にどれだけ誤検知が混ざっているかということに着目したのがRP曲線である。限られた人数しか正の予測をしてはならないような場面に向いていると考えられる。負例全体の数はあまり興味がない点がROC曲線とは異なる。例えば、あるサービス訴求を行いたいが、予算の関係で対象者を絞らなくてはならないといった場面である。こうしたとき、無駄打ちをなくすためにはPrecisionが高い予測を行う必要がある。
...
LightGBMを詳しく説明すると本書の目的を逸脱してしまうので、アルゴリズムの詳細は割愛する。
...
=== モデル評価
==== ROC曲線
斜め45度線からどれだけ上振れしているかが、学習の精度をあらわす。
...
==== RP曲線
水平な線で示されている、データセットそのものの正例割合からモデルによりどれだけ多く正例を言い当てられるかがこの図からわかる。

...
しかし、予測値のキャリブレーションが不十分で予測値の大小と加入確率の大小で逆転している場合、予測値の大きい方を選択すると実は加入確率が低い方を選択してしまっていたということが起こりうる。
...
正例負例の予測値の分布がそれぞれパラメトリックに数式で得られる場合、訓練データに含まれない説明変数でも正例負例の確率密度を計算してその割合を見積もることができる。

テストデータに対してもこのようなことを行えば、確率値にキャリブレーションされた予測値を得ることができる。

=== グラフィカルモデルによる説明
上記の説明をグラフィカルモデルに落とし込むと@<img>{graph}のようになる。
説明変数Xや目的変数Tはそれぞれ、訓練データとテストデータに分割することができ、テストデータをX'、T'などと表す。@<img>{graph}の他のシンボルは@<table>{params_graph}にまとめてある。
...
//image[dataset][訓練データ（左）、テストデータ（右）][scale=0.3]{
//}
...
また、テストデータの予測値S'についても、パラメータCを用いて正例割合Y'を算出できる。
...
また、テストデータの正解T'がわからなくても、X'からY'を算出し、TとYの関係からT'の予測を行うことができる。
...
ここに、loglossを損失関数にしている二値分類モデルでは不均衡データの学習が難しい理由がある。
ある程度負例にフィッティングしてしまうと、Loglossが小さくなってしまい学習の勾配がつかなくなるわけである。
...
前章で見たように、この予測値はAUCが訓練データに対して0.9以上あり、かなり学習できているように思われるが、相関図では予測値=正例割合となる対角線からは大きく離れている。
この乖離を無くすのが本章での目的である。
...
正例負例それぞれについて確率密度を計算して、訓練データでの正例負例の数を考慮することで以下のように正例割合を予測することができる。
...
結果として予測値を正例割合にキャリブレーションした相関図が@<img>{cal_score_ratio_plot}である。@<img>{corr_plot}と同様に予測値を0.01刻みでまとめてある。
